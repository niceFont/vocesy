			...
			{fetched ?
				<div> {exists ?
					<Container style={{marginBottom: 100}}>
						<Container style={{padding: 5, border: "1px solid #e3e6e4"}}>
							<Row className="justify-content-between" style={{padding: 5}}>
								<Col className="my-auto"> 
									<h6 className="text-capitalize" style={{fontWeight: "bold", margin: "0 0 0 50px" }}>{data[0].title + "-cards"}</h6>
								</Col>
								<Col>
									<Link href={`/play/${props.slug}?uv=${settings.userValidation}`}>
										<Button style={{fontWeight: "bold"}} disabled={!data[0].front} block variant="outline-success" size="md">START</Button>
									</Link>
								</Col>
								<Col className="text-right my-auto">
									<Button variant="light" size="sm" style={{margin: "0 5px 0 0"}} onClick={() => toggleAdding(true)}>Add</Button>
									<Button variant="light" size="sm" style={{margin: "0 50px 0 0"}} onClick={()=>toggleSetting(s => !s)}>Settings</Button>
								</Col>
							</Row>
							{setting &&	
							<Row>
								<Col style={{margin: "20px 0 20px 0"}}>
									<div className='text-center custom-control custom-switch' >
										<input type='checkbox' className='custom-control-input' id='customSwitches' readOnly onChange={() => { 
											changeSettings(settings => Object.assign({...settings, userValidation: !settings.userValidation}))
										}}/>
										<label className='custom-control-label' htmlFor='customSwitches'>Toggle User Validation</label>
									</div>
						...
						<Row className="justify-content-center" style={{padding: 10, border: "1px solid #e3e6e4"}}>
						</Row>
