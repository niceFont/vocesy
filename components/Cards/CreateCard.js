...
export class CreateCard extends React.PureComponent{
	...
	async _handleSave() {
		...
		if (!CheckIfEditorEmpty(this.state.front) && !CheckIfEditorEmpty(this.state.back)) {
			this.setState({
				sending: true 
			})
			try {
	...
	handleChange(event, editor) {
		this.setState({ charCount: editor.getData().length })
		
		if (this.state.charCount > this.state.charLimit) {
			this.setState({error: "Only a Maximum of 200 Characters allowed."})
		} else {
